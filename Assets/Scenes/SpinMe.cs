...
public class SpinMe : MonoBehaviour
{
	...
	void Update ()
    {
        ...
        float perminuteperrotation = Time.deltaTime / 60 * 360;

		float xDegreesPerFrame = perminuteperrotation * xRotationsPerMinute;
		transform.RotateAround (transform.position, transform.right, xDegreesPerFrame);

		float yDegreesPerFrame = perminuteperrotation * yRotationsPerMinute;
		transform.RotateAround (transform.position, transform.up, yDegreesPerFrame);

		float zDegreesPerFrame = perminuteperrotation * zRotationsPerMinute;
